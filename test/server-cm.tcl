...
proc makeGUI {{parent {}}} {
    global upsize_gui updata downsize downsize_gui downdata
    
    # create up buffer (where user can write to)
    set maxrow [expr ($upsize_gui-1)/16+1]
    ...
    set maxrow [expr ($downsize_gui-1)/16+1]
    set baserow $row
    for {set row 0} {$row < $maxrow} {incr row} {
        for {set col 0} {$col < 16} {incr col} {
